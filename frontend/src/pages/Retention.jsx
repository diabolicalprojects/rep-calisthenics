...
const Retention = () => {
    ...
    const sendAutomation = async (member, type) => {
        ...
        const msgs = {
            Reactivación: `¡Hola ${member.name}! 💪 Te extrañamos en REP. Tenemos una oferta especial para que vuelvas. 🔥`,
            Incentivo: `¡Hola ${member.name}! 👋 Notamos tu ausencia. ¡Te regalamos una clase de cortesía para tu regreso! 🔋`,
            Default: `¡Hola ${member.name}! 👋 Queremos saber cómo vas con tus entrenamientos en REP. ⚡`
        };
    ...
    if (loading) return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 16 }}>
            <RefreshCw size={32} className="text-orange animate-spin" />
            <p className="text-muted">Analizando patrones de abandono...</p>
        </div>
    );
    ...
    return (
        <div className="animate-fade-in">
            ...
            <ModuleMetricBar stats={[
                ...
                { label: 'Ingreso Vulnerable', value: fmtCurrency(metrics.moneyAtRisk), color: 'var(--color-accent-orange)' },
                { label: 'Recuperables', value: metrics.reactivationPotential, color: 'var(--color-success)' },
                { label: 'Health Score', value: '92%', color: '#4da6ff' },
            ]} />
            ...
            <div className="glass-panel stagger-2 pos-flex-center-mobile" style={{ display: 'flex', gap: 40, padding: 32, flexWrap: 'wrap', alignItems: 'center', marginBottom: 24 }}>
                <div style={{ height: 180, width: 260 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData}>
                            ...
                            <YAxis hide />
                            <RechartsTooltip contentStyle={{ background: '#111', border: 'none', borderRadius: 12 }} />
                ...
                <div style={{ flex: 1, minWidth: 300 }}>
                    ...
                    <div style={{ display: 'flex', gap: 32 }}>
                        {chartData.map((d, i) => (
                            <div key={d.name}>
                                <div style={{ fontSize: 24, fontWeight: 900, color: i === 0 ? 'var(--color-danger)' : i === 1 ? 'var(--color-accent-orange)' : 'var(--color-success)' }}>{d.value}</div>
            ...
            <div className="dashboard-grid-main">
                <div className="glass-panel mobile-full" style={{ padding: 0, overflow: 'hidden' }}>
                    <div style={{ padding: 24, borderBottom: '1px solid var(--color-glass-border)' }}>
                        <h3 style={{ fontSize: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                            <ShieldAlert size={18} className="text-orange" /> Miembros en Alerta
                        </h3>
                    </div>
                                    ...
                                    <tr key={m.id} onClick={() => setSelectedMember(m)}>
                                        <td data-label="Miembro">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                                <div className="avatar-small" style={{ background: m.riskScore > 80 ? 'var(--color-danger)15' : 'var(--color-accent-orange)15', color: m.riskScore > 80 ? 'var(--color-danger)' : 'var(--color-accent-orange)' }}>{m.name[0]}</div>
                                                <div>
                                        ...
                                        <td data-label="Riesgo">
                                            <div style={{ width: 100 }}>
                                                <div style={{ height: 6, background: '#222', borderRadius: 3, overflow: 'hidden' }}>
                                                    <div style={{ height: '100%', width: `${m.riskScore}%`, background: m.riskScore > 80 ? 'var(--color-danger)' : 'var(--color-accent-orange)' }}></div>
                                                </div>
                                                <div style={{ fontSize: 9, fontWeight: 900, marginTop: 4, color: m.riskScore > 80 ? 'var(--color-danger)' : 'var(--color-accent-orange)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    <div className="glass-panel" style={{ background: 'var(--color-accent-orange)05' }}>
                        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                            <Bell size={16} className="text-orange" /> Automatizaciones
                        </h3>
            ...
            <BaseModal isOpen={!!selectedMember} onClose={() => setSelectedMember(null)} title="Análisis de Cliente">
                {selectedMember && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        <div style={{ textAlign: 'center', padding: 24, background: 'rgba(0,0,0,0.2)', borderRadius: 16 }}>
                            <div className="avatar-large" style={{ margin: '0 auto 16px', background: 'var(--color-accent-orange)15', color: 'var(--color-accent-orange)' }}>{selectedMember.name[0]}</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                            <div className="glass-panel" style={{ padding: 16 }}>
                                ...
                                <div style={{ fontSize: 20, fontWeight: 900, color: selectedMember.riskScore > 80 ? 'var(--color-danger)' : 'var(--color-accent-orange)' }}>{selectedMember.riskScore}%</div>
                            </div>
                            <div className="glass-panel" style={{ padding: 16 }}>
                        ...
                        <div className="glass-panel" style={{ padding: 20, borderLeft: '4px solid var(--color-accent-orange)' }}>
                            <h4 style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                                <Target size={14} /> Recomendación IA
                            </h4>
                            <p style={{ fontSize: 14, color: 'var(--color-text-muted)', lineHeight: '1.5' }}>
                                {selectedMember.status === 'Inactivo' 
                                    ? 'Cliente perdido. Sugerimos campaña de recupero con 20% de descuento en la siguiente mensualidad.' 
                                    : 'Alta probabilidad de abandono. Enviar incentivo motivacional o clase de cortesía para retomar el ritmo.'}
                            </p>
                        </div>
